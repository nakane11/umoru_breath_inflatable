...
void loop() {
  ...
  if(state==0) {
    ...
    if (count >= 5000) {
      if (!add_air_executed) {
        add_air();           // 一度だけ add_air() を実行
        add_air_executed = true;
      }else {
        remove_air();        // 5000カウント後にremove_air()を実行
        add_air_executed = false;  // 次のループでadd_air()が実行されるようにリセット
      }
      count = 0;           // カウントをリセットしてremove_air()のタイミングを測る
    }
  }
  delay(10);
  nh.spinOnce();
